...
public abstract class MinRunLockableFilter implements Filter
{
	...
	Optional<Security> filteredSecurity = minRunLocked ? security : executeNonMinRunLockableFilter(security, context); // no filtering needed if minRunLocked
	return filteredSecurity;
    }

    protected abstract Optional<Security> executeNonMinRunLockableFilter(Optional<Security> security, ConcurrentApplicationContext context);
}
